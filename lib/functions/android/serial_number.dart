...
class SerialNumberWidget extends StatefulWidget {
  ...
  @override
  _SerialNumberWidgetState createState() => _SerialNumberWidgetState();
}
...
class _SerialNumberWidgetState extends State<SerialNumberWidget> {
  ...
  Future<void> getSerialNumber() async {
    try {
      ...
      setState(() {
        _serialNumber = context.tr('unknownOrNotAndroidNotice');
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_serialNumber == "") getSerialNumber();
    return Scaffold(
      appBar: AppBar(
        title: Text(context.tr('serialNumberLongName')),
        actions: [
          IconButton(
            ...
            onPressed: () {
              showModalBottomSheet(
                  ...
                  builder: (context) => InfoModalBottomSheet(
                      name: context.tr('serialNumberLongName'),
                      description: context.tr('serialNumberLongDescription'),
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(_serialNumber, style: const TextStyle(fontSize: 24)),
          ],
        ),
      ),
    );
