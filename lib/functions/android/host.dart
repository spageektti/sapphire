...
class _HostWidgetState extends State<HostWidget> {
  ...
  Future<void> getHost() async {
    try {
      ...
      setState(() {
        _host = androidInfo.host;
      });
    } catch (e) {
      setState(() {
        _host = context.tr('unknownOrNotAndroidNotice');
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(context.tr('hostLongName')),
        actions: [
          IconButton(
            ...
            onPressed: () {
              showModalBottomSheet(
                  ...
                  builder: (context) => InfoModalBottomSheet(
                      name: context.tr('hostLongName'),
                      description: context.tr('hostLongDescription'),
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(_host, style: const TextStyle(fontSize: 24)),
          ],
        ),
      ),
    );
